...
export default function DoctorLiveQueue() {
  ...
  const { getQueueForDoctor, callPatient, startConsultation, completeConsultation, skipPatient } = useQueue()
  const { getDoctorById, refetchDoctors } = useDirectory()
  ...
  const paused = Boolean(getDoctorById(doctorId)?.queuePaused)
  ...
  async function handleStartConsultation(token) {
